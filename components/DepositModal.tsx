...
interface DepositModalProps {
  isOpen: boolean;
  onClose: () => void;
  onDeposit: (amount: number, onStatus: (msg: string) => void) => Promise<void>;
...
export const DepositModal: React.FC<DepositModalProps> = ({ isOpen, onClose, onDeposit, currentBalance }) => {
  ...
  const handleDeposit = async () => {
    ...
    if (!isNaN(val) && val > 0) {
      ...
      try {
        await onDeposit(val, (msg) => setStatus(msg));
        setAmount('');
        onClose();
      } catch (e) {
        console.error(e);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-dark-800 rounded-2xl border border-dark-700 w-full max-w-md p-6 shadow-2xl">
        ...
        <div className="mt-4 text-center text-xs text-gray-500">
           Transaction 1: Approve USDC Access <br/>
           Transaction 2: Deposit to Vault
        </div>
      </div>
    </div>
  );
